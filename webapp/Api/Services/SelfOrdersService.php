...
class SelfOrdersService
{
    ...
    {
        $this->selfOrder = $this->model('api/Self_order', 'selfOrder');
        $this->connectCloudService = $this->make('api/Aws_connect_cloud_service', null, 'connectCloudService');
    }
    ...
    public function createOrders($params = [], $headers = [])
    {
        ...
        if (!isset($params['multiple']) || $params['multiple'] == 'no') {
            return $this->selfOrder->updateOrInsert($params, $headers);
        }
        // update the orders
    ...
    public function getOrdersList($params = [], $headers = [])
    {
        ...
        if (isset($params['update_list']) && count($params['update_list']) > 0 ) {
            $conditions = [
                ...
                'pos_store_code'   => $headers['store_code'],
                'order_status !='  => ORDER_STATUS_CANCELLED,
            ];

            $resultUpdate = $this->_updateOrdersList($params['update_list'], $conditions);

            if ( !empty($resultUpdate['data_push']) ) {
                $this->connectCloudService->pushNotification($resultUpdate['data_push'], $headers);
            }
        }
    ...
    public function getPrintStatus($params = [], $headers = [])
    {
        ...
        $conditions = [
            ...
            'pos_store_code'   => $params['store_code'] ?? $headers['store_code'],
